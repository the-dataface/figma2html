...
export const generateOutputSvelte = (config, assets, variables) => {
	...
	let resizerJs = config.includeResizer
		? getResizerScript(containerId, true)
		: '';
	...
	let startComment = `<!-- Generated by figma2html on ${getDateTimeStamp()} -->\r`,
  ...
  `;
	...
	if (config.includeGoogleFonts)
		fontScript = generateGoogleFontScript(fontList);
...
export const generateOutputHtml = (config, assets, variables) => {
	...
	let linkSrc = config.clickableLink || '';
	let resizerJs = config.includeResizer
		? getResizerScript(containerId, false)
		: '';
	let widthRange = getWidthRange(assets);
	let html,
		js,
...
export const generateFrameCss = (frame, frameId) => {
	...
	const css = `${id} { position: relative; overflow: hidden; display: none; }\r`;
	return css;
};
...
export const generatePageCss = (containerId, config) => {
	...
	const computedStyles = Object.entries(selectors).forEach(
		([selector, properties]) => {
  ...
  \r`;
	...
	if (textData) {
		textData.forEach((text) => {
			let el = ``;
			let effect =
				text.effect.length > 0 ? generateTextEffect(text.effect) : '';

			let style = `style="`;
			style += `top: ${text.y}; left: ${text.x}; opacity: ${text.opacity}; width: ${text.width}; ${effect}`;

			if (text.rotation !== 0)
				style += ` transform: rotate(${text.rotation}deg); transform-origin: left top;`;
			style += `"`;

			let classes = `class="`;
			if (text.classes)
				text.classes.forEach(
					(c, i) =>
						(classes += `${c}${
							i < text.classes.length - 1 ? ' ' : ''
						}`)
				);
			classes += `"`;
...
export const generateTextEffect = (effects) => {
	...
	let dropShadows = effects.filter(
...
export const createSpan = (
	...
	if (applyStyles) {
		styleProps.forEach((prop) => {
			if (segment[prop])
				styleTag += convertStyleProp(prop, segment[prop]);
		});
	}
