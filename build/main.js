...
var styleProps = { convert, fontList, styles: styles$1 };
...
var convertTextFrames = (textFrames, frameWidth, frameHeight) => {
    ...
    return textFrames.map((textFrame, i) => {
        ...
        segments.forEach((seg, i) => {
            ...
            const isOtherWeight = isBaseStyle && stylesObject['font-weight'] !== 400 && stylesObject['font-weight'] !== 700
...
var page = ({ containerId, config, frameCss }) => {
...
var frame = ({ node, filename, widthRange, altText, config, variables }) => {
    let inlineStyle = '';
    let pStyle;
...
var fonts = (fontList) => {
    ...
    fontList.forEach((font) => {
        let family = font.family, weight = font.style === 'Italic'
            ? weightLookup['Regular']
            : weightLookup[font.style.replace(' Italic', '')], style = font.style.includes('Italic') ? '1,' : '0,', styleWeight = `${style}${weight}`;
...
StoredViews.get = async () => {
    ...
    return _views !== null && _views !== void 0 ? _views : defaults.views;
};
...
StoredVariables.get = async () => {
    ...
    const variablesNode = figma.currentPage.findChild((node) => node.type === 'TEXT' && node.name === 'f2h-variables');
    if (!!(variablesNode === null || variablesNode === void 0 ? void 0 : variablesNode.characters)) {
...
StoredVariables.writeVariables = async () => {
    ...
    const existingVariables = figma.currentPage.findChild((node) => node.type === 'TEXT' && node.name === 'f2h-variables');
    if (!!(existingVariables === null || existingVariables === void 0 ? void 0 : existingVariables.characters)) {
    ...
    figma.loadFontAsync({ family: 'Inter', style: 'Regular' }).then(() => {
        ...
        const nodes = figma.currentPage.findAll((node) => node.name.startsWith('#') || node.name === 'f2h-settings');
...
StoredConfig.get = async () => {
    ...
    return _config !== null && _config !== void 0 ? _config : defaults.config;
};
...
StoredConfig.writeSettings = async (config) => {
    ...
    const settings = figma.currentPage.findChild((node) => node.type === 'TEXT' && node.name === 'f2h-settings');
    if (settings) {
    ...
    figma.loadFontAsync({ family: 'Inter', style: 'Regular' }).then(() => {
        ...
        const nodes = figma.currentPage.findAll((node) => node.name.startsWith('#') || node.name === 'f2h-variables');
        // get furthest point to the right
        const maxRight = nodes.reduce((max, node) => {
            return Math.max(max, node.x + node.width);
        }, 0);
...
StoredConfig.loadSettings = async () => {
    ...
    const settingsNode = figma.currentPage.findChild((node) => node.type === 'TEXT' && node.name === 'f2h-settings');
...
class TempFrame {
    constructor() {
        ...
        this.remove = () => {
            var _e;
            (_e = this.frame) === null || _e === void 0 ? void 0 : _e.remove();
            this.frame = undefined;
        };
    }
}
...
const getExportables = () => {
    const nodes = figma.currentPage.findAll((node) => node.name.match(/^#\d+px$/) && node.type === 'FRAME' && node.parent === figma.currentPage);
