...
const createSettingsBlock = (config) => {
...
const convert = (prop, value) => {
  ...
  if (prop === "lineHeight")
    return ` line-height: ${value.unit === "AUTO" ? "normal" : value.unit === "PERCENT" && value.value > 0 ? value.value / 100 : value.value + "px"};`;
  if (prop === "letterSpacing")
    return ` letter-spacing: ${value.unit === "PERCENT" && value.value > 0 ? value.unit / 100 : value.value + "px"};`;
  if (prop === "fills" && value.length > 0)
    return ` color: rgba(${value[0].color.r * 255}, ${value[0].color.g * 255}, ${value[0].color.b * 255}, ${value[0].opacity}); mix-blend-mode: ${value[0].blendMode.toLowerCase()};`;
};
const fontList = [];
const styles$1 = (segment, props) => {
  let styleString = "", stylesObject;
  props.forEach((prop) => {
    if (segment[prop])
      styleString += convert(prop, segment[prop]);
  });
  stylesObject = jsYaml.load(styleString.replaceAll("; ", "\n").replaceAll("undefined", ""));
  return { styleString, stylesObject };
};
const styleProps = { convert, fontList, styles: styles$1 };

const dashify = (str) => str.replace(/[^\w\s]/gi, "").replace(/\s+/g, "-").toLowerCase();

const trim = (s) => s.replace(/^[\s\uFEFF\xA0\x03]+|[\s\uFEFF\xA0\x03]+$/g, "");

const convertTextFrames = (textFrames, frameWidth, frameHeight) => {
  const props = [
    "fontName",
    "fontWeight",
    "fontSize",
  ...
  return textFrames.map((textFrame, i) => {
    ...
    segments.forEach((seg, i2) => {
      const stylesObject = styleProps.styles(seg, props).stylesObject, styleString = styleProps.styles(seg, props).styleString;
      const isBaseStyle = i2 === 0 ? true : stylesObject["font-family"] === textSegments[0].stylesObject["font-family"] && stylesObject["font-size"] === textSegments[0].stylesObject["font-size"] && stylesObject["letter-spacing"] === textSegments[0].stylesObject["letter-spacing"] && stylesObject["color"] === textSegments[0].stylesObject["color"] && stylesObject["line-height"] === textSegments[0].stylesObject["line-height"] && stylesObject["mix-blend-mode"] === textSegments[0].stylesObject["mix-blend-mode"] && stylesObject["text-decoration"] === textSegments[0].stylesObject["text-decoration"] && stylesObject["text-transform"] === textSegments[0].stylesObject["text-transform"];
    ...
    const baseStyle = {
      tag,
      style: textSegments[0].styleString.replace("font-weight: 700", "font-weight: 400").replace("font-style: italic", "font-style: normal")
    };
    ...
    return {
      ...
      width: textFrame.textAutoResize === "WIDTH_AND_HEIGHT" ? "auto" : `${textFrame.width.toFixed(2)}px`,
...
const page = ({ containerId, config, frameCss }) => {
  ...
  let computedStyles = Object.entries(selectors).map(([selector, properties]) => {
    return `	${selector} { ${Object.entries(properties).map(([property, value]) => `
		${property}: ${value};`).join("")}
	}`;
  }).join("\n\n");
  computedStyles += `
...
const css = { frame: frame$1, page, textEffect };

const roundTo = (num, precision) => Math.round(num * Math.pow(10, precision)) / Math.pow(10, precision);

const attrs = (obj) => Object.entries(obj).filter(([, value]) => value).map(([key, value]) => `${key}="${value}"`).join(" ");
const styles = (obj) => Object.entries(obj).filter(([, value]) => value).map(([key, value]) => `${key}: ${value};`).join(" ");
const stringify = { attrs, styles };

const span = (segment, variables, styleTextSegments) => {
  let el = ``;
  ...
  if (!segment.isBaseStyle && styleTextSegments)
    el += `<span style="${segment.styleString}">`;
...
const frame = ({ node, filename, widthRange, altText, config, variables }) => {
  let inlineStyle = "";
  let pStyle;
...
`;
  if (textData) {
    ...
    pStyle = baseStyles.sort(
      (a, b) => baseStyles.filter((v) => v === a).length - baseStyles.filter((v) => v === b).length
    ).pop();
    ...
    textData.forEach((text) => {
      ...
      text.segments.forEach((segment, i) => {
        ...
        if (notNewElement) {
          ...
          els.push({
            tag: config.applyHtags && ["h1", "h2", "h3", "h4", "h5", "h6"].includes(trim(text.class)) ? trim(text.class) : "p",
            segments: [segment],
...
const fonts = (fontList) => {
  ...
  fontList.forEach((font) => {
    let family = font.family, weight = font.style === "Italic" ? weightLookup["Regular"] : weightLookup[font.style.replace(" Italic", "")], style = font.style.includes("Italic") ? "1," : "0,", styleWeight = `${style}${weight}`;
...
const resizer = (containerId, isSvelte) => {
  const resizer = (containerId2) => {
...
const js = { resizer, fonts };

const widthRange = (assets) => {
...
const html = ({ config, assets, variables }) => {
  let html = ``;
  let containerId = `${config.syntax}-box`;
  let frameCss = assets.map(
    ({ node, filename }) => frame({
      node,
      filename,
...
<!-- End figma2html generated content - ${timestamp()} -->`;
    ...
    if (config.includeResizer)
      html += `		${js.resizer(containerId, true)}
`;
    html += `<!-- Generated by figma2html on ${timestamp()} -->`;
    html += `
...
const log = (...args) => console.log("[figma2html]", ...args);
...
StoredVariables.writeVariables = async () => {
  ...
  figma.loadFontAsync({ family: "Inter", style: "Regular" }).then(() => {
    ...
    textNode.characters = jsYaml.dump(variables);
    textNode.x = xPos;
    textNode.y = minTop;
...
StoredConfig.writeSettings = async (config) => {
  ...
  figma.loadFontAsync({ family: "Inter", style: "Regular" }).then(() => {
    ...
    let textNode = figma.createText();
    textNode.characters = jsYaml.dump(formatConfig(config));
    textNode.x = xPos;
    textNode.y = minTop;
    textNode.name = "f2h-settings";
  });
};
StoredConfig.loadSettings = async () => {
  const settingsNode = figma.currentPage.findChild(
    (node) => node.type === "TEXT" && node.name === "f2h-settings"
  );
  if (!!settingsNode?.characters) {
    const config = parseConfig(jsYaml.load(settingsNode.characters));
    await _StoredConfig.set(config);
  }
};
...
const getExportables = () => {
  const nodes = figma.currentPage.findAll(
    (node) => node.name.match(/^#\d+px$/) && node.type === "FRAME" && node.parent === figma.currentPage
  );
...
const getFile = async (config, assets, variables) => {
  return {
...
const getAssets = async (exportables, config, previewSettings) => {
  tempFrame.create();
  let assets = [];
  for (let exportable of exportables) {
    ...
    const { settings } = createSettingsBlock(
      previewSettings.isFinal ? baseExportConfig : {
    ...
    try {
      asset.data = await modifiedNode.exportAsync(settings);
    } catch (exportable2) {
      log(exportable2);
    }
    assets.push(asset);
...
const withModificationsForText = (node) => {
  ...
  frameNodes.filter((node2) => {
    return node2.findAllWithCriteria({ types: ["TEXT"] }).length > 0;
  });
...
const withModificationsForExport = (node, config) => {
  ...
  config.testingMode ? textNodes.forEach((node2) => node2.opacity = 0.5) : textNodes.forEach((node2) => node2.visible = false);
  return node;
};
const refreshPreview = async (config, variables) => {
...
const generateExport = async (config, variables) => {
  const exportables = getExportables();
...
figma.ui.onmessage = async (message) => {
  ...
  switch (type) {
    ...
    case "view":
      if (message.views)
        storedViews = await StoredViews.set(message.views);
      break;
    case "resize":
      if (message.size)
        storedSize = await StoredSize.set(message.size);
      figma.ui.resize(storedSize.w, storedSize.h);
      break;
    case "config":
      figma.ui.postMessage({
